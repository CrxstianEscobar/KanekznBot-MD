...
const { makeInMemoryStore, DisconnectReason, useMultiFileAuthState, MessageRetryMap, fetchLatestBaileysVersion, makeCacheableSignalKeyStore } = await import('baileys')
const { CONNECTING } = ws
const { chain } = lodash
...
console.log('🌙 Shadow Bot no tiene Sub-Bots vinculados.');
return;
}
...
console.log(chalk.bold.greenBright(`🟢 Shadow Bot conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🥀 Error al iniciar Shadow Bot: `, error))
}
})();
